...
const migrations: Migration[] = [
  ...
  {
    id: '013',
    name: 'add_outreach_tables',
    up: (db) => {
      console.log('[Migration 013] Adding outreach campaigns, leads, and profiles tables...');
      ...
      db.exec(`CREATE INDEX IF NOT EXISTS idx_outreach_campaigns_workspace ON outreach_campaigns(workspace_id, status, created_at DESC)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_outreach_leads_campaign ON outreach_leads(campaign_id, status, created_at DESC)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_outreach_leads_workspace ON outreach_leads(workspace_id, status)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_outreach_leads_postcode ON outreach_leads(postcode, business_name)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_outreach_lead_profiles_lead ON outreach_lead_profiles(lead_id)`);
    }
  },
  {
    id: '014',
    name: 'add_site_generation_tables',
    up: (db) => {
      console.log('[Migration 014] Adding site_templates and generated_sites tables...');
      ...
      db.exec(`CREATE INDEX IF NOT EXISTS idx_site_templates_vertical ON site_templates(vertical, template_type)`);
      db.exec(`CREATE INDEX IF NOT EXISTS idx_generated_sites_lead ON generated_sites(lead_id)`);
