...
function parsePlanningSpec(task: Task): PlanningSpecPreview | null {
  const planningSpecRaw = (task as Task & { planning_spec?: string | null }).planning_spec;
  if (!planningSpecRaw) {
    return null;
  }
  try {
    const parsed = JSON.parse(planningSpecRaw) as PlanningSpecPreview;
...
function TaskCard({ task, onDragStart, onClick, isDragging }: TaskCardProps) {
  ...
  const isRecentlyActive = isInProgress && (Date.now() - new Date(task.updated_at).getTime()) < 90000;
  const planningSpec = parsePlanningSpec(task);
  const planHeadline = planningSpec?.objective || planningSpec?.summary;
  const nextSteps = (planningSpec?.plan_steps ?? []).slice(0, 2);
  const approvalCount = (planningSpec?.approvals_required ?? []).length;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {/* Execution plan preview (when planning spec exists) */}
        {planningSpec && (
          <div className="mb-3 py-2 px-3 bg-mc-bg-tertiary/40 rounded border border-mc-border/40">
            <p className="text-[11px] uppercase tracking-wide text-mc-text-secondary mb-1">Execution Plan</p>
            {planHeadline && (
            ...
            {nextSteps.length > 0 && (
              <ol className="list-decimal list-inside text-xs text-mc-text-secondary space-y-1">
            ...
            {approvalCount > 0 && (
              <p className="text-[11px] text-mc-accent-yellow mt-2">
                {approvalCount} approval requirement{approvalCount > 1 ? 's' : ''}
              </p>
            )}
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-mc-border/20">
          <div className="flex items-center gap-2">
            ...
            {isRecentlyActive && (
              <span className="text-[10px] uppercase tracking-wide text-mc-accent-green bg-mc-accent-green/10 px-1.5 py-0.5 rounded border border-mc-accent-green/30">
                active now
              </span>
            )}
          </div>
