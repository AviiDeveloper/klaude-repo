...
function getPlanningBootstrapUrl(taskId: string): string {
  const configuredOrigin = process.env.MISSION_CONTROL_INTERNAL_ORIGIN;
  if (configuredOrigin && configuredOrigin.trim().length > 0) {
    return `${configuredOrigin.replace(/\/+$/, '')}/api/tasks/${taskId}/planning`;
  }

  const port = process.env.MISSION_CONTROL_PORT || process.env.PORT || '3001';
  return `http://127.0.0.1:${port}/api/tasks/${taskId}/planning`;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const status = 'planning'; // Lead intake authority path
    ...
    run(
      ...
      [
        ...
        lead.id,
        id,
        `Task "${body.title}" entered Lead intake`,
        JSON.stringify({ lead_agent_id: lead.id }),
        now,
      ],
    );
    ...
    // Always enqueue in Lead intake first.
    intakeTask({
      taskId: id,
      workspaceId,
      triageSummary: 'Awaiting lead triage and delegation.',
    });

    // Keep planning bootstrap behavior for planning status.
    fetch(getPlanningBootstrapUrl(id), {
      method: 'POST',
