...
async function getMessagesFromOpenClaw(
  sessionKey: string
): Promise<Array<{ role: string; content: string; error?: string }>> {
  try {
    ...
    const messages: Array<{ role: string; content: string; error?: string }> = [];
    ...
    for (const msg of result.messages || []) {
      if (msg.role === 'assistant') {
        ...
        if (textContent?.text) {
          ...
          messages.push({
            role: 'assistant',
            content: `PLANNER_ERROR: ${msg.errorMessage}`,
...
export async function GET(
  ...
  try {
    ...
    if (!lastAssistantMessage && task.planning_session_key && messages.length > 0) {
      ...
      if (openclawMessages.length > 0) {
        ...
        if (newAssistant) {
          ...
          if (newAssistant.error) {
            plannerError = newAssistant.error;
          }
        }
      }
    }
    ...
    if (lastAssistantMessage) {
      ...
      } else if (lastAssistantMessage.content.startsWith('PLANNER_ERROR:')) {
        plannerError = lastAssistantMessage.content.replace('PLANNER_ERROR:', '').trim();
      }
    }
...
- Caller asks concise multiple-choice clarification questions.
- Final output must support orchestrator requirements:
  - objective
  - plan_steps (3 to 8)
  - assigned_agents
  - side_effects
  ...
  "options": [
    ...
    for (let i = 0; i < 30; i++) { // Poll for up to 30 seconds
      ...
      if (transcriptMessages.length > 0) {
        ...
        if (lastAssistant) {
          if (lastAssistant.error || lastAssistant.content.startsWith('PLANNER_ERROR:')) {
            const errorMessage = (lastAssistant.error || lastAssistant.content.replace('PLANNER_ERROR:', '').trim()).trim();
            return NextResponse.json(
