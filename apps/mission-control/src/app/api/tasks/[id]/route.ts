...
export async function PATCH(
  ...
  try {
    ...
    const nextAssignedAgentId = body.assigned_agent_id;
    ...
    if (nextAssignedAgentId !== undefined && nextAssignedAgentId !== existing.assigned_agent_id) {
      const latestDelegation = getLatestDelegation(id);
      const isLeadDelegation =
        latestDelegation &&
        latestDelegation.delegated_to_agent_id === nextAssignedAgentId &&
        latestDelegation.status !== 'blocked';
      if (!isLeadDelegation) {
        return NextResponse.json(
          { error: 'Direct assignment blocked: Lead delegation required' },
          { status: 403 },
        );
      }
      updates.push('assigned_agent_id = ?');
      values.push(nextAssignedAgentId);

      if (nextAssignedAgentId) {
        const agent = queryOne<Agent>('SELECT name FROM agents WHERE id = ?', [nextAssignedAgentId]);
        if (agent) {
          run(
            `INSERT INTO events (id, type, agent_id, task_id, message, created_at)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [uuidv4(), 'task_assigned', nextAssignedAgentId, id, `"${existing.title}" assigned to ${agent.name}`, now]
          );
