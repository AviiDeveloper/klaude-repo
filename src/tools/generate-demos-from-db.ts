...
function selectLeads(data: Record<string, any>, count: number): LeadSelection[] {
  ...
  return candidates.map((q: any) => {
    const profile = profileMap.get(q.lead_id) ?? {};
    return {
      lead_id: q.lead_id,
      ...
      instagram_json: profile.instagram_json ?? q.instagram_json,
    };
  });
}
...
async function generateDemo(lead: LeadSelection, idx: number, total: number) {
  ...
  // Compose — pass instagram_json through the lead data
  console.log(`${tag} Composing site (with vision)...`);
  const t1 = Date.now();
  const composeLead = {
    ...lead,
    ...lead.profile,
    instagram_json: lead.instagram_json,
  };
    ...
    JSON.stringify(
      {
        ...
        ai_tokens_used: site.ai_tokens_used ?? null,
        ai_cost_usd: site.ai_cost_usd ?? null,
        images_used: site.images_used ?? 0,
...
// ---------------------------------------------------------------------------
// Simple server to view results (serves asset images too)
// ---------------------------------------------------------------------------
...
function startServer(port: number) {
  const server = createServer((req, res) => {
    ...
    // Serve asset images via the same URL pattern the composer uses
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
      ...
      const rows = files.map((f) => {
        ...
        return `<tr>
          ...
          <td>${meta.ai_generated ? `AI (${meta.images_used ?? 0} imgs, $${(meta.ai_cost_usd ?? 0).toFixed(3)})` : "Template"}</td>
      ...
      res.end(`<!DOCTYPE html><html><head><title>Demo Sites</title>
        ...
        <table><tr><th>Business</th><th>Type</th><th>Score</th><th>QA</th><th>Method</th><th>Generated</th></tr>
